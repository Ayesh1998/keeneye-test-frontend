...
const CartComponent = (props) => {
  ...
  useEffect(() => {
    ...
    setTotalPriceAndTotalInCart(productsAddedToCartFiltered);
  }, [productsModified]);

  //setting total Price and total items in the cart
  const setTotalPriceAndTotalInCart = (productsAddedToCartFiltered) => {
    let totalPrice = 0;
    let totalItemsInCart = 0;
    productsAddedToCartFiltered.map((product) => {
      totalPrice += product.noAddedToCart * product.price;
      totalItemsInCart += product.noAddedToCart;
    });
    Number(totalPrice).toFixed(2);
    setTotal(totalPrice);
    setTotalAddedToCart(totalItemsInCart);
  }
    ...
    <div className="cart-dropdown">
      <div className="cart-items">
        {productsAddedToCart.length ? (
          productsAddedToCart.map((cartItem) => (
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <span className="red-text empty-cart">
              Cart is Empty
            </span>
        ...
        <div>
          <div className="row" style={{ marginTop: "5px" }}>
            <span className="col-6" style={{ fontSize: "11px" }}>
              Total Amount({totalAddedToCart})
            </span>
            <span className="col-6" style={{ fontSize: "11px", color: "red" }}>
              LKR {  Number(total).toFixed(2)}
            </span>
          </div>
          <div className="container bottom_border" style={{ margin: "10px 0px", borderColor: "grey" }}/>
          <div className="row" style={{ marginTop: "5px" }}>
            <span className="col-6" style={{ fontSize: "11px" }}>
              Total Price
            </span>
          ...
          <div className="row" style={{ marginTop: "10px" }}>
            <div className="col-4 checkout-button-in-cart">
              Go to checkout
            </div>
            <div className="col-4"/>
            <div className="col-3 close-button-in-cart" onClick={() => dispatch(toggleDropdown())}>
              Close
            </div>
          </div>
        </div>
